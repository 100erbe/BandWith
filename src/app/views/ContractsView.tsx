...
Generated by BandWith
  ...
  return (
    <motion.div
      ...
      className="fixed inset-0 z-[70] bg-background overflow-y-auto overflow-x-hidden flex flex-col"
      ...
      <div className="flex-1 px-4 pb-20 overflow-y-auto">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20">
            ...
            <span className="text-xs font-bold text-foreground/40 uppercase tracking-wide mb-1">No contracts found</span>
            <span className="text-[10px] font-medium text-foreground/30 uppercase">Create your first contract or rider</span>
          </div>
        ) : (
          <>
            {/* Stats Row */}
            <div className="flex gap-5 mb-10">
              <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }} className="flex-1 flex flex-col gap-1 items-start">
                ...
                <span className="text-[10px] font-medium text-foreground/40 uppercase">DOCUMENTS</span>
              </motion.div>
              <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="flex-1 flex flex-col gap-1 items-start">
                ...
                <span className="text-[10px] font-medium text-foreground/40 uppercase">COMPLETED</span>
              </motion.div>
            </div>
            ...
            {filteredContracts.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-10">
                <span className="text-[10px] font-medium text-foreground/40 uppercase">No documents match your filter</span>
              </div>
            ) : (
          ...
          >
            ...
            <div className="flex-1 overflow-y-auto px-6 pb-32">
              <div className="space-y-6">
                ...
                >
                  <label className="text-[10px] font-bold uppercase tracking-widest text-foreground/40 block mb-3">Document Type</label>
                  <div className="flex gap-3">
                    <button
                      ...
                      className={cn(
                        ...
                        formType === 'contract'
                          ? "bg-black text-white"
                          : "bg-white/50 text-foreground/50 hover:bg-white"
                      )}
                    >
                ...
                >
                  <label className="text-[10px] font-bold uppercase tracking-widest text-foreground/40 block mb-2">Document Title</label>
                  <input
                    type="text"
                    value={formTitle}
                ...
                >
                  <label className="text-[10px] font-bold uppercase tracking-widest text-foreground/40 block mb-2">Event Name</label>
                  <input
                    type="text"
                    value={formEventName}
                ...
                >
                  <label className="text-[10px] font-bold uppercase tracking-widest text-foreground/40 block mb-2">Client / Venue</label>
                  <input
                    type="text"
                ...
                >
                  <label className="text-[10px] font-bold uppercase tracking-widest text-foreground/40 block mb-2">Event Date</label>
                  <div className="relative">
                    ...
                    />
                    <Calendar className="absolute right-4 top-1/2 -translate-y-1/2 w-5 h-5 text-foreground/30 pointer-events-none" />
                  </div>
                </motion.div>
                ...
                >
                  <label className="text-[10px] font-bold uppercase tracking-widest text-foreground/40 block mb-2">Additional Notes</label>
                  <textarea
                    value={formNotes}
                    onChange={(e) => setFormNotes(e.target.value)}
                    placeholder="Any special requirements, technical specifications, or notes..."
                    rows={4}
                    className="w-full bg-white/50 rounded-2xl p-4 text-base font-medium text-black placeholder:text-foreground/30 focus:outline-none focus:ring-2 focus:ring-black resize-none"
                  />
                </motion.div>
              </div>
            </div>
          ...
          >
            ...
            >
              <div className="flex items-start justify-between mb-6">
                <div>
                  <span className="text-[10px] font-bold uppercase text-foreground/40 mb-2 block">
              ...
              <div className="space-y-4">
                <div className="p-4 bg-black/[0.03] rounded-2xl">
                  <p className="text-xs font-bold text-foreground/40 uppercase mb-1">Event</p>
                  <p className="text-lg font-bold text-black">{viewingContract.event_name}</p>
                </div>
                
                <div className="grid grid-cols-2 gap-4">
                  <div className="p-4 bg-black/[0.03] rounded-2xl">
                    <p className="text-xs font-bold text-foreground/40 uppercase mb-1">Client</p>
                    <p className="font-bold text-black">{viewingContract.client_name}</p>
                  </div>
                  <div className="p-4 bg-black/[0.03] rounded-2xl">
                    <p className="text-xs font-bold text-foreground/40 uppercase mb-1">Event Date</p>
                    <p className="font-bold text-black">{formatDate(viewingContract.event_date)}</p>
                  </div>
                </div>

                <div className="p-4 bg-black/[0.03] rounded-2xl">
                  <p className="text-xs font-bold text-foreground/40 uppercase mb-1">Type</p>
